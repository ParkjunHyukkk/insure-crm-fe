...
import { Search, Plus, Eye, Edit, Trash2, Phone, Calendar, Filter } from "lucide-react"

export function CustomerManagement({ customers, setCustomers, onCustomerSelect }) {
  ...
  const filteredCustomers = customers.filter((customer) => {
    ...
    const matchesFilter = filterProduct === "all" || customer.product === filterProduct
  ...
  const handleAddCustomer = () => {
    const customer = {
      ...newCustomer,
      id: Date.now(),
      age: new Date().getFullYear() - new Date(newCustomer.birthDate).getFullYear(),
      ...
      <Card>
            ...
            <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
              ...
              <DialogContent className="max-w-2xl">
                <DialogHeader>
                  ...
                  <DialogDescription>새로운 고객의 정보를 입력해주세요.</DialogDescription>
                </DialogHeader>
                <div className="grid grid-cols-2 gap-4 py-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => setNewCustomer((prev) => ({ ...prev, joinDate: e.target.value }))}
                    />
                  </div>
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => setNewCustomer((prev) => ({ ...prev, renewalDate: e.target.value }))}
                    />
                  </div>
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(value) => setNewCustomer((prev) => ({ ...prev, product: value }))}
                    >
                      <SelectTrigger>
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(value) => setNewCustomer((prev) => ({ ...prev, paymentMethod: value }))}
                    >
                      <SelectTrigger>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => setNewCustomer((prev) => ({ ...prev, premium: e.target.value }))}
                      placeholder="150000"
                    />
                  </div>
        ...
        <CardContent>
                ...
                {filteredCustomers.map((customer) => {
                  ...
                  return (
                    <TableRow key={customer.id}>
                      ...
                      <TableCell>
                        <Badge variant={getPaymentMethodBadge(customer.paymentMethod)}>{customer.paymentMethod}</Badge>
                      </TableCell>
                      <TableCell>{customer.premium.toLocaleString()}원</TableCell>
                      <TableCell>
                        <div className="flex items-center gap-2">
                          <Calendar className="h-4 w-4 text-gray-400" />
                          <span className={daysUntilRenewal <= 30 ? "text-red-600 font-medium" : ""}>
                            D-{daysUntilRenewal}
                          </span>
                        </div>
                      </TableCell>
                      <TableCell>
                        <div className="flex items-center gap-2">
                          <Button size="sm" variant="outline" onClick={() => onCustomerSelect(customer)}>
          ...
          {filteredCustomers.length === 0 && (
            <div className="text-center py-8 text-gray-500">검색 조건에 맞는 고객이 없습니다.</div>
          )}
        </CardContent>
      </Card>
    </div>
