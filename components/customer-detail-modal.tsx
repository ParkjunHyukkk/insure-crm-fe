...
import { User, Phone, Calendar, CreditCard, FileText, MessageSquare, Edit, Save, X } from "lucide-react"
    ...
    <Dialog open={true} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>고객의 상세 정보 및 상담 기록을 확인할 수 있습니다.</DialogDescription>
        </DialogHeader>
            ...
            <CardContent className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">{customer.age}세</p>
                </div>
              </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-medium">{customer.premium.toLocaleString()}원</p>
                  <p className="text-sm text-muted-foreground">월 보험료</p>
                </div>
              </div>

              <div className="flex items-center gap-3">
                <Badge variant={customer.paymentMethod === "자동이체" ? "default" : "secondary"}>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex items-center justify-between p-3 border rounded-lg">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">{customer.renewalDate}</p>
                </div>
                <Badge variant={daysUntilRenewal <= 30 ? "destructive" : "default"}>D-{daysUntilRenewal}</Badge>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-lg flex items-center justify-between">
                메모
                <Button size="sm" variant="outline" onClick={() => (isEditing ? handleSaveMemo() : setIsEditing(true))}>
                  {isEditing ? (
                    <>
            ...
            <CardContent>
              {isEditing ? (
                ...
                <p className="text-sm text-muted-foreground">{memo || "등록된 메모가 없습니다."}</p>
              )}
            </CardContent>
          </Card>
        </div>
          ...
          <CardContent>
            <div className="space-y-4">
              {consultationHistory.map((consultation) => (
                <div key={consultation.id} className="flex items-start gap-4 p-4 border rounded-lg">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <Badge variant={consultation.result === "완료" ? "default" : "secondary"}>
                        {consultation.result}
                      </Badge>
                    </div>
